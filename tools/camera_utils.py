...
def get_interpolated_poses_many(
    ...
    for idx in range(poses.shape[0]):
        ...
        pose_b = poses[(idx + 1)%len(poses)].cpu().numpy()
        poses_ab = get_interpolated_poses(pose_a, pose_b, steps=steps_per_transition)
        traj += poses_ab
        k_interp += get_interpolated_k(Ks[idx], Ks[(idx + 1)%len(poses)], steps=steps_per_transition)
...
def rotation_matrix_between(a: Float[Tensor, "3"], b: Float[Tensor, "3"]) -> Float[Tensor, "3 3"]:
    ...
    return torch.eye(3).to(a) + torch.sin(theta) * skew_sym_mat + (1 - torch.cos(theta)) * (skew_sym_mat @ skew_sym_mat)
...
def auto_orient_and_center_poses(
    ...
    if method == "pca":
        ...
        rotation = rotation_matrix_between(up, torch.Tensor([0, 0, 1]).to(up))
